<?php
  ...
  if ($_SESSION['Admin']=="" || $_SESSION['Admin']==null ) 
  {
    ...
    $conexion = new mysqli('localhost','root','','drogueria_pps',3306);
	...
	if($resultado->num_rows > 0 ){
		...
		$objPHPExcel->getProperties()->setCreator("Droguería PPs") //Autor
							 ...
							 ->setTitle("Reporte usuarios Droguería PPs")
		...
		$titulosColumnas = array('Identificacion', 'Nombre', 'Apellido', 'Dirección', 'Teléfono-Celular', 'Correo', 'Nombre de Usuario', 'Contraseña');
		...
		while ($fila = $resultado->fetch_array()) {
			$objPHPExcel->setActiveSheetIndex(0)
        		    ->setCellValue('A'.$i,  $fila['identificacionCliente'])
		            ->setCellValue('B'.$i,  $fila['nombreCliente'])
        		    ->setCellValue('C'.$i,  $fila['apellidoCliente'])
        		    ->setCellValue('D'.$i,  $fila['direccionCliente'])
        		    ->setCellValue('E'.$i,  $fila['telefonoCliente'])
        		    ->setCellValue('F'.$i,  $fila['correoCliente'])
        		    ->setCellValue('G'.$i,  $fila['nombre_Usuario'])
            		->setCellValue('H'.$i, utf8_encode($fila['passwordCliente']));
					$i++;
		}
